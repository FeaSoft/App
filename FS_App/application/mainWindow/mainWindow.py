...
from dataModel import (
    ...
    OutputDatabase, Mesh, ElementTypes, BodyLoad, SurfaceSet, Pressure, SurfaceTraction
)
...
class MainWindow(MainWindowShell):
    ...
    def __init__(self) -> None:
        ...
        self._menuBarQueryMesh.triggered.connect(self.onMenuBarQueryMesh)                             # type: ignore
        self._menuBarDebugTraceback.triggered.connect(self.onMenuBarDebugTraceback)                   # type: ignore
        self._menuBarHelpAbout.triggered.connect(self.onMenuBarHelpAbout)                             # type: ignore
    ...
    def enablePicking(
        ...
        self._toolBarInteractionPickSingle.setEnabled(pickSingle)
        self._toolBarInteractionPickMultiple.setEnabled(pickMultiple)
        if (
            (not pickSingle and self._toolBarInteractionPickSingle.isChecked()) or
            (not pickMultiple and self._toolBarInteractionPickMultiple.isChecked())
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def onModelTreeSelection(self) -> None:
        ...
        match dataObject:
            ...
            case SurfaceSet():
                stopPicking = False
                self.enablePicking(False, True, 'Points', lambda x, y: self.onViewportPick(dataObject, x, y))
                self._modelViewport.info.setText(1, 'Edit Surface Set')
                self._modelViewport.info.setText(0, 'Use Viewport Pickers to Add/Remove Surfaces')
                self._modelViewport.setSelectionRenderObject(dataObject, (1.0, 0.0, 0.0), render=False)
            case Section():
            ...
            case Pressure():
                if dataObject.surfaceSetName != '<Undefined>':
                    self._modelViewport.setSelectionRenderObject(
                        dataObject, (0.75, 0.5, 1.0), self._modelDatabase, render=False
                    )
            ...
            case BodyLoad():
                if dataObject.elementSetName != '<Undefined>':
                    self._modelViewport.setSelectionRenderObject(
                        dataObject, (1.0, 0.0, 1.0), self._modelDatabase, render=False
                    )
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def onViewportPick(
        self,
        dataObject: NodeSet | ElementSet | SurfaceSet,
        indices: Sequence[int],
        remove: bool
    ) -> None:
        '''On viewport picking action performed.'''
        if not self._modelDatabase: return
        if isinstance(dataObject, NodeSet) or isinstance(dataObject, ElementSet):
            if remove: dataObject.remove(indices)
            else: dataObject.add(indices)
        else:
            surfaces = self._modelDatabase.convertNodeIndicesToSurfaces(indices)
            if remove: dataObject.remove(surfaces)
            else: dataObject.add(surfaces)
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def onMenuBarDebugTraceback(self) -> None:
        '''On Menu Bar > Debug > Traceback.'''
        sys.tracebacklimit = 1000 if self._menuBarDebugTraceback.isChecked() else 0

    def onMenuBarQueryMesh(self) -> None:
        '''On Menu Bar > Query > Mesh.'''
        match self._module:
            case 'Preprocessor':
                if not self._modelDatabase: raise RuntimeError('a model database must first be opened')
                mesh: Mesh = self._modelDatabase.mesh
            case 'Visualization':
                if not self._outputDatabase: raise RuntimeError('an output database must first be opened')
                mesh: Mesh = self._outputDatabase.mesh
        elementTypeCounts: dict[str, int] = {x.name: 0 for x in ElementTypes}
        for element in mesh.elements: elementTypeCounts[element.elementType.name] += 1
        print('Mesh statistics:')
        print(' '*4 + f'Number of nodes: {len(mesh.nodes)}')
        print(' '*4 + f'Number of elements: {len(mesh.elements)}')
        for name, count in elementTypeCounts.items():
            if count > 0:
                print(' '*8 + f'Number of {name}: {count}')

    def onMenuBarHelpAbout(self) -> None:
